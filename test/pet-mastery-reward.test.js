...
import { CHALLENGE_REWARD_DEFS, SemesterGame } from "../game-engine.js";
...
test("精通宠物的伙伴路线冻结为一次性校园币且不再增加无效羁绊", () => {
  ...
  const pending = game.prepareChallengeCombatReward({ affix: "backlog", enemyId: "phoneSpirit" });
...
test("未精通仍获得羁绊并触发里程碑，可领取宠物蛋时蛋优先于精通回退", () => {
  ...
  nearMastery.prepareChallengeCombatReward({ affix: "deadline", enemyId: "phoneSpirit" });
  ...
  assert.equal(nearMastery.pet.bond, 26);
  ...
  eggPriority.prepareChallengeCombatReward({ affix: "earlyClass", enemyId: "sleepyBug" });
