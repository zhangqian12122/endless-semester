...
test("八名普通敌人获得互不重复的动作身份且不覆盖特殊敌人", () => {
  ...
  assert.equal(new Set(resolved).size, 8);
...
test("每种普通敌人都拥有不同的攻击与技能节拍，并在 0.55 秒内回正", () => {
  ...
  assert.equal(new Set(attackFingerprints).size, 8, "八名敌人的攻击动作应全部不同");
  assert.equal(new Set(skillFingerprints).size, 8, "八名敌人的技能动作应全部不同");
});
