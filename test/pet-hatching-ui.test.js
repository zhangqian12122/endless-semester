...
test("宠物只能在新游戏开始界面选择，休息点只展示本局锁定伙伴", () => {
  ...
  assert.match(appSource, /new SemesterGame\(Date\.now\(\), archetypeId, startingPetId, availablePets, startingPersonaId\)/);
...
test("孵化流程在挑战奖励、地图、休息点与战后结算中都有明确反馈", () => {
  ...
  assert.doesNotMatch(appSource, /petIncubationRecapHtml|renderCombatResult/);
  assert.doesNotMatch(styles, /\.incubation-recap\s*\{/);
});
...
test("孵化与开局选择样式包含移动端布局，入口缓存版本保持同步", () => {
  ...
  assert.match(index, /styles\.css\?v=1\.8\.63/);
  assert.match(index, /app\.js\?v=1\.8\.63/);
});
