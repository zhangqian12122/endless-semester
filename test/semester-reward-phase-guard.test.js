...
test("主力升级必须来自总结阶段且有候选时不能空结算", () => {
  ...
  game.cardCombatUses[target.uid] = 3;
  enterSummaryUpgrade(game);

  assert.equal(game.completeCurrentSemester(), false, "存在合格主力牌时不能跳过升级");
  assert.equal(game.completeCurrentSemester("missing-uid"), false);
  assert.equal(game.completeCurrentSemester(other.uid), false, "未达标卡牌不能冒充主力候选");
  assert.equal(game.awaitingNextSemester, false);
  assert.equal(game.pendingSemesterReward?.stage, "summaryUpgrade");
  assert.equal(target.upgraded, false);
  assert.equal(other.upgraded, false);
});

test("合法主力 UID 只升级一次并原子完成本学期", () => {
  const game = new SemesterGame(708, "gemini");
  const eligible = game.deck.filter((card) => !card.upgraded).slice(0, 2);
  for (const card of eligible) game.cardCombatUses[card.uid] = 3;
  enterSummaryUpgrade(game);
...
test("无人达标时可完成总结，下一学期只能从等待状态进入一次", () => {
  ...
  enterSummaryUpgrade(game);
  assert.deepEqual(game.eligibleSummaryCards(), []);
