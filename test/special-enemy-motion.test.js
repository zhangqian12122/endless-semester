...
test("特殊敌人动作 profile 区分卷王冲刺、期末重压与发卷展开", () => {
  ...
  assert.equal(resolveProfile({ enemyId: "sleepyBug", playerDamage: 5 }), "default-attack");
  assert.equal(resolveProfile({ enemyId: "homeworkBlob", intentName: "堆作业" }), "default-skill");
});
...
test("特殊动作沿用 windup 和 impact 标签并保持普通敌人原节奏", () => {
