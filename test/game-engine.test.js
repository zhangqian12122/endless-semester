...
import { ARCHETYPE_TRIAL_DEFS, CHALLENGE_AFFIX_DEFS, CHALLENGE_REWARD_DEFS, CHALLENGE_RULES, ITEM_REWARD_FALLBACK_GOLD, TAROT_DEFS, SemesterGame, STARTING_DECK, cardDefinition, startingDeckFor } from "../game-engine.js";
import { ACHIEVEMENT_DEFS, ARCHETYPE_CARD_IDS, BOSS_ITEM_IDS, CARD_ART_DEFS, CARD_DEFS, DEFAULT_PET_ID, ENCHANTMENT_DEFS, ENEMY_DEFS, ITEM_DEFS, NORMAL_ENEMY_IDS, PET_DEFS, PET_TALENT_DEFS, PUBLIC_REWARD_CARD_IDS, REGULAR_ITEM_IDS } from "../game-data.js";
...
test("嘉豪、宕机鸭与代表敌人保持独立素材并保留双入口灰盒降级", () => {
  ...
  assert.equal(new Set(independentRoleAssets).size, independentRoleAssets.length, "嘉豪、鸭的两张素材与三名敌人必须保留彼此独立的路径");

  for (const [label, asset] of Object.entries({ jiahao: jiahaoAsset, duckBattle: duckAssets.battle, duckIcon: duckAssets.icon })) {
  ...
  assert.match(appSource, /function renderBattlePet\(\)[\s\S]*?characterAssetHtml\(pet\.assets\.battle, "character-asset battle-pet-asset", `\$\{pet\.name\}战斗形象`\)[\s\S]*?class="avatar-fallback battle-pet-fallback pet-full-fallback"/, "战场必须使用宕机鸭完整图并保留完整 CSS 鸭降级");
  assert.match(appSource, /function renderPetFace\(\)[\s\S]*?characterAssetHtml\(pet\.assets\.icon, "character-asset pet-icon-asset"\)[\s\S]*?class="avatar-fallback pet-icon-fallback"/, "充能头像必须使用独立鸭头像并保留 CSS 鸭降级");
...
test("玩家可见宠物文案完成宕机鸭迁移，同时保留兼容用内部标识", () => {
  ...
  assert.equal(ACHIEVEMENT_DEFS.gooseCall.icon, "鸭");
  assert.equal(ACHIEVEMENT_DEFS.gooseCall.name, "鸭来！");
  assert.match(ACHIEVEMENT_DEFS.gooseCall.text, /宕机鸭/);
...
test("十件随身物品公开触发频率，文案边界与实际规则一致", () => {
  assert.equal(Object.keys(ITEM_DEFS).length, 10);
...
test("十件随身物品全部使用克制的小尺寸独立图标", () => {
  ...
  assert.match(styles, /\.item-icon img \{[^}]*filter: saturate\(\.58\)/);
...
test("随身物品图鉴按稀有度完整筛选并保持只读入口", () => {
  ...
  assert.deepEqual(itemLibraryIds(ITEM_DEFS, "rare"), ["petSnack"]);
...
test("战斗冲击反馈只展示实际结算差值并区分攻防与敌方行动", () => {
  const attack = battleFeedbackFromDelta(
    ...
    { id: 7, kind: "card", label: "左右横跳", cardPlayed: true }
  );
  ...
  const enemy = battleFeedbackFromDelta(
    ...
    { kind: "enemy", label: "作业团 · 堆作业", playerBlockAbsorbed: 7, enemyBlockGain: 9 }
  );
  ...
  const statusEnemy = battleFeedbackFromDelta(
    ...
    { playerHp: 40, playerBlock: 0, enemyHp: 30, enemyBlock: 0 },
    { kind: "enemy", effectParts: ["加入弃牌堆 1 张待办"] }
  );
  ...
  assert.match(styles, /\.feedback-card\.feedback-guard \.feedback-streak, \.feedback-card\.feedback-guard \.feedback-burst \{ display: none; \}/);
...
test("战斗演出使用固定 GSAP 时间轴并为减少动态效果提供可靠降级", () => {
  ...
  assert.match(appSource, /action === "play-card"[\s\S]*?const motionOrigin = captureBattleMotionOrigin\([\s\S]*?queueBattleFeedback\("card"[\s\S]*?\{ cardPlayed: true, motionOrigin \}/);
  ...
  assert.match(styles, /\.battle-feedback\.gsap-driven\b/);
});
...
test("宠物充能入口与能量组合固定在一起，并保留技能详情操作", () => {
  ...
  assert.match(appSource, /<div class="energy-companion-stack">[\s\S]*?<article class="pet-companion-token[\s\S]*?<div class="energy-orb">/);
  ...
  assert.match(appSource, /class="pet-skill \$\{petReady \? "ready" : ""\}" data-action="pet-skill"/);
});
...
test("结束回合提示区分安全、可防掉血与无视护甲的致命伤害", () => {
  ...
  assert.match(appSource, /enemyIntentTokenHtml\(intent, enemyResolution\)/);
...
test("敌方结算阶段冻结本次行动与实际结果，不提前显示下一意图", () => {
  ...
  const appSource = readFileSync(new URL("../app.js", import.meta.url), "utf8");
  const styles = readFileSync(new URL("../styles.css", import.meta.url), "utf8");
  assert.match(appSource, /function enemyIntentTokenHtml\(intent, resolution = null\)/);
  assert.match(appSource, /enemyIntentTokenHtml\(intent, enemyResolution\)/);
...
test("敌方非伤害行动返回真实结果，并区分施加、加入与物品抵挡", () => {
  ...
  const appSource = readFileSync(new URL("../app.js", import.meta.url), "utf8");
  const styles = readFileSync(new URL("../styles.css", import.meta.url), "utf8");
  assert.match(appSource, /function enemyActionEffects\(result\)/);
...
test("宠物牌与羁绊路线足够集中时识别为鸭鸭协同", () => {
  ...
  assert.equal(analysis.primary.label, "鸭鸭协同");
