...
function allowNextSemester(game) {
  game.week = 16;
  game.awaitingNextSemester = true;
}

function winFinalExamForRewards(game) {
...
function winEliteForRewards(game, usedCardUids = []) {
  ...
  assert.ok(pending);
  game.startCombat(pending.enemyId, pending.modifiers);
  for (const uid of usedCardUids) game.combat.usedCardUids.add(uid);
  game.combat.enemy.hp = 0;
  assert.equal(game.checkCombatEnd(), "won");
  assert.equal(game.completePendingCombatStart(), true);
}

function winChallengeForRewards(game, { enemyId = null, affix = null } = {}) {
  if (!game.tarotId) assert.equal(game.chooseTarot("chariot"), true);
  const preferredWeeks = game.week <= 7
    ? [3, 4, 5, 6, 7]
    : [9, 10, 11, 12, 13, 14, 15];
  const challengeEntry = preferredWeeks
    .flatMap((week) => game.semesterPlan[week].map((node) => ({ week, node })))
    .find(({ node }) => node.challenge)
    || game.semesterPlan
      .flatMap((nodes, week) => nodes.map((node) => ({ week, node })))
      .find(({ node }) => node.challenge);
  assert.ok(challengeEntry, "测试路线应包含挑战战");

  game.week = challengeEntry.week;
  const resolvedEnemyId = enemyId || "homeworkBlob";
  challengeEntry.node.enemy = resolvedEnemyId;
  if (affix) challengeEntry.node.affix = affix;
  const resolvedAffix = challengeEntry.node.affix;
  const pending = game.prepareCombatStart(resolvedEnemyId, "challenge", {
    challenge: true,
    hpMultiplier: CHALLENGE_RULES.hpMultiplier,
    damageMultiplier: CHALLENGE_RULES.damageMultiplier,
    affix: resolvedAffix
  });
  assert.ok(pending, "真实挑战战应能建立开战检查点");
  game.startCombat(pending.enemyId, pending.modifiers);
  game.combat.enemy.hp = 0;
  assert.equal(game.checkCombatEnd(), "won");
  assert.equal(game.completePendingCombatStart(), true);
  const reward = game.prepareChallengeCombatReward({
    affix: resolvedAffix,
    enemyId: resolvedEnemyId
  });
  assert.ok(reward, "真实挑战胜利应能准备奖励");
  return reward;
}
...
test("嘉豪、宕机鸭与八名敌人保持独立素材并保留灰盒降级", () => {
  ...
  assert.match(appSource, /function renderBattlePet\(\)[\s\S]*?pet-full-fallback[\s\S]*?characterAssetHtml\((?:asset|pet\.assets\.battle), "character-asset battle-pet-asset", `\$\{pet\.name\}战斗形象`\)/, "战场必须使用当前宠物完整图并保留 CSS 鸭降级");
  assert.match(appSource, /function renderPetFaceFor\(petId\)[\s\S]*?pet-icon-fallback[\s\S]*?characterAssetHtml\((?:asset|pet\.assets\.icon), "character-asset pet-icon-asset"\)/, "充能头像必须使用当前宠物独立头像并保留 CSS 降级");
...
test("顶栏校园币、卡组与物品使用统一的小尺寸原生图标", () => {
  ...
  assert.match(styles, /@media \(max-width: 700px\) \{[\s\S]*?\.topbar \{[^}]*flex-wrap: nowrap;[\s\S]*?\.resource-stat \{ gap: 4px; \}/);
});
...
test("卡牌图鉴按公共池、三种星座和状态牌完整筛选", () => {
  ...
  assert.deepEqual(CARD_LIBRARY_FILTERS, ["all", "public", "aries", "gemini", "cancer", "summoner", "status"]);
  assert.deepEqual(allIds, Object.keys(CARD_DEFS));
  assert.equal(publicIds.length, PUBLIC_REWARD_CARD_IDS.length + 4);
  ...
  assert.deepEqual(cardLibraryIds(CARD_DEFS, "summoner"), ["summonPaperCrane", "allPresent", "coverDuty", "scatterAfterClass"]);
...
test("战斗演出使用固定 GSAP 时间轴并为减少动态效果提供可靠降级", () => {
  ...
  assert.match(appSource, /action === "play-card"[\s\S]*?queueBattleFeedback\("card"[\s\S]*?cardPlayed: true,[\s\S]*?cardType:[\s\S]*?cleanseApplied/);
  assert.doesNotMatch(appSource, /action === "play-card"[\s\S]*?captureBattleMotionOrigin\(button,\s*"card"\)/);
...
test("战斗双方共享校园舞台并把中央战报降为次要信息", () => {
  const styles = readFileSync(new URL("../styles.css", import.meta.url), "utf8");
  const backgroundAsset = "assets/scenes/classroom-battle-v3.webp";
  ...
  assert.match(styles, new RegExp(`url\\("${backgroundAsset.replaceAll("/", "\\/")}"\\) center 55% \\/ cover no-repeat`));
...
test("宕机鸭满充能后花费 1 能量出手，随后清空充能等待下一轮蓄力", () => {
  ...
  assert.equal(game.combat.petSkillUses, 1);
  assert.equal(game.combat.petUsed, false, "战斗进行中不能再用旧字段锁死已重新充能的宠物");
  assert.match(game.combat.log.at(-1), /宕机鸭·重启猛啄：造成 7 伤害/);
  assert.deepEqual(game.usePetSkill(), { ok: false, reason: "宠物充能未满" });
});

test("宠物出手后可跨回合重新充能再次出手，同回合攻击充能仍只触发一次", () => {
  const game = new SemesterGame(506, "cancer");
  game.items = ["petSnack"];
  game.startCombat("finalExam");
  assert.equal(game.pet.charge, 1, "宠物零食仍提供开战充能");

  const firstAttack = putCardInHand(game, "textbookStrike");
  assert.equal(game.playCard(firstAttack.uid).ok, true);
  assert.equal(game.pet.charge, game.pet.maxCharge);
  const firstEnergy = game.combat.energy;
  assert.deepEqual(game.usePetSkill(), { ok: true });
  assert.equal(game.combat.energy, firstEnergy - PET_DEFS.offlineDuck.skill.energyCost);
  assert.equal(game.pet.charge, 0);

  const sameTurnAttack = putCardInHand(game, "textbookStrike");
  assert.equal(game.playCard(sameTurnAttack.uid).ok, true);
  assert.equal(game.pet.charge, 0, "宠物出手不能重置本回合一次的攻击充能限制");
  assert.deepEqual(game.usePetSkill(), { ok: false, reason: "宠物充能未满" });

  assert.equal(game.endTurn().ok, true);
  const nextTurnAttack = putCardInHand(game, "textbookStrike");
  assert.equal(game.playCard(nextTurnAttack.uid).ok, true);
  assert.equal(game.pet.charge, 1);
  assert.equal(game.endTurn().ok, true);
  const followingTurnAttack = putCardInHand(game, "textbookStrike");
  assert.equal(game.playCard(followingTurnAttack.uid).ok, true);
  assert.equal(game.pet.charge, game.pet.maxCharge);

  const secondEnergy = game.combat.energy;
  assert.deepEqual(game.usePetSkill(), { ok: true });
  assert.equal(game.combat.energy, secondEnergy - PET_DEFS.offlineDuck.skill.energyCost);
  assert.equal(game.pet.charge, 0);
  assert.equal(game.combat.petSkillUses, 2);
  assert.equal(game.stats.petUses, 2);
  assert.equal(game.combat.petUsed, false);

  game.combat.enemy.hp = 0;
  assert.equal(game.checkCombatEnd(), "won");
  assert.equal(game.combat.petUsed, true, "旧布尔字段在结算时仍表示本场曾使用宠物");
  assert.equal(game.combatSummary().petUsed, true);
  const restored = SemesterGame.fromJSON(game.toJSON());
  assert.equal(restored.stats.petUses, 2);
  assert.equal(restored.pet.charge, 0);
});

test("所有已实现宠物都能按各自充能上限循环出手", () => {
  const petIds = Object.keys(PET_DEFS);
  for (const petId of petIds) {
    const game = new SemesterGame(508, "cancer", petId, petIds);
    game.startCombat("finalExam");
    game.combat.energy = 5;
    game.combat.maxEnergy = 5;
    const preview = game.petSkillPreview();
    const hpBefore = game.combat.enemy.hp;
    const blockBefore = game.combat.playerBlock;

    for (let use = 1; use <= 2; use += 1) {
      game.pet.charge = game.pet.maxCharge;
      assert.deepEqual(game.usePetSkill(), { ok: true }, `${petId} 第 ${use} 次满充能应能出手`);
      assert.equal(game.pet.charge, 0);
    }

    assert.equal(game.combat.enemy.hp, hpBefore - preview.damage * 2);
    assert.equal(game.combat.playerBlock, blockBefore + preview.block * 2);
    assert.equal(game.combat.petSkillUses, 2);
    assert.equal(game.stats.petUses, 2);
  }
});

test("宠物技能失败不会消耗能量、充能或增加出手计数", () => {
  const game = new SemesterGame(507, "gemini");
  game.startCombat("finalExam");
  game.pet.charge = game.pet.maxCharge;
  ...
  const before = {
    ...
    enemyHp: game.combat.enemy.hp,
    combatUses: game.combat.petSkillUses,
    totalUses: game.stats.petUses
  };

  assert.deepEqual(game.usePetSkill(), { ok: false, reason: "需要 1 点能量" });
  assert.deepEqual({
    energy: game.combat.energy,
    charge: game.pet.charge,
    enemyHp: game.combat.enemy.hp,
...
test("白羊赤焰刻印让攻击牌每段伤害加 2，并可与升级叠加", () => {
  ...
  assert.deepEqual(cardDefinition(strike).enchantment, ENCHANTMENT_DEFS.ariesFlame);
...
test("护崽路线的护甲会在每次重新充能出手时正常生效", () => {
  ...
  game.pet.charge = game.pet.maxCharge;
  assert.equal(game.usePetSkill().ok, true);
  assert.equal(game.combat.playerBlock, 10);
  assert.equal(game.combat.petSkillUses, 2);
});
...
test("叼笔记路线立即抽牌并为下回合提供抽牌奖励", () => {
  ...
  assert.deepEqual(game.petSkillPreview().talent, PET_TALENT_DEFS.scout);
});
...
test("教学考试与固定事件保持原位，挑战和怪谈继续使用完整普通敌人池", () => {
  ...
  assert.equal(plan[5].find((node) => node.challenge).enemy, "clubMegaphone");
  assert.equal(plan[10].find((node) => node.challenge).enemy, "clubMegaphone");
  ...
  assert.equal(game.randomEnemy(), "clubMegaphone");
  assert.deepEqual([...eventPool], NORMAL_ENEMY_IDS);
});

test("同一种子生成相同路线，且固定周、核心战斗周与补给保底成立", () => {
  ...
  const combatChoiceWeeks = new Set([4, 6, 11, 14, 15]);
  for (const week of openWeeks) {
    const nodes = first.semesterPlan[week];
    assert.equal(nodes.length, 2);
    if (combatChoiceWeeks.has(week)) {
...
test("挑战战复盘会固定试炼与路线状态，专属牌候选刷新后不重抽", () => {
  ...
  const pending = winChallengeForRewards(game, { enemyId: "sleepyBug", affix: "deadline" });
...
test("挑战物品候选会保存，物品收齐时 45 币替代奖励只结算一次", () => {
  ...
  game.week = 6;
  winChallengeForRewards(game, { affix: "backlog" });
  const goldBefore = game.gold;
  ...
  winChallengeForRewards(collected, { affix: "earlyClass" });
...
test("只有挑战手机精的既有物品路线会固定静音手机，普通池与其他挑战不会混入", () => {
  ...
  const pending = winChallengeForRewards(phoneChallenge, { affix: "backlog", enemyId: "phoneSpirit" });
  ...
  winChallengeForRewards(alreadyOwned, { affix: "deadline", enemyId: "phoneSpirit" });
  ...
  winChallengeForRewards(otherChallenge, { affix: "earlyClass", enemyId: "sleepyBug" });
...
test("挑战手机精招牌物品在书包满时复用原有替换流程", () => {
  ...
  winChallengeForRewards(game, { affix: "backlog", enemyId: "phoneSpirit" });
...
test("旧版手机精挑战奖励存档会补全静音手机且不打断已开始的普通物品替换", () => {
  ...
  routeGame.week = 6;
  winChallengeForRewards(routeGame, { affix: "backlog", enemyId: "phoneSpirit" });
  ...
  itemGame.week = 7;
  winChallengeForRewards(itemGame, { affix: "deadline", enemyId: "phoneSpirit" });
  ...
  winChallengeForRewards(replacementGame, { affix: "earlyClass", enemyId: "phoneSpirit" });
...
test("挑战宠物路线跨过羁绊里程碑时保存完成阶段且不会重复增加羁绊", () => {
  ...
  game.week = 9;
  winChallengeForRewards(game, { affix: "deadline" });
  const goldBefore = game.gold;
...
test("三种塔罗在休息点各提供一次有代价的专属共鸣", () => {
  ...
  allowNextSemester(hermit);
  assert.equal(hermit.startNextSemester(), true);
...
test("力量共鸣不会向精通且满血的伙伴出售空收益，仍保留治疗与成长用途", () => {
  const masteredFull = new SemesterGame(515, "gemini");
  masteredFull.chooseTarot("strength");
  masteredFull.gold = 50;
  masteredFull.hp = masteredFull.maxHp;
  masteredFull.pet.bond = 25;
  masteredFull.pet.talent = "guardian";
  masteredFull.pet.talentLevel = 3;
  const masteredFullBefore = masteredFull.toJSON();

  assert.deepEqual(masteredFull.tarotRestStatus(), {
    ...TAROT_DEFS.strength.rest,
    tarotId: "strength",
    available: false,
    reason: "伙伴已精通且生命已满，本次共鸣没有可获得的收益"
  });
  assert.equal(masteredFull.tarotRestPreview().available, false);
  assert.deepEqual(masteredFull.resolveTarotRest(), {
    ok: false,
    reason: "伙伴已精通且生命已满，本次共鸣没有可获得的收益"
  });
  assert.deepEqual(masteredFull.toJSON(), masteredFullBefore, "禁用的共鸣不能扣币、增加羁绊或消耗本学期次数");

  const masteredInjured = new SemesterGame(516, "gemini");
  masteredInjured.chooseTarot("strength");
  masteredInjured.hp = masteredInjured.maxHp - 6;
  masteredInjured.pet.bond = 25;
  masteredInjured.pet.talent = "guardian";
  masteredInjured.pet.talentLevel = 3;
  assert.equal(masteredInjured.tarotRestStatus().available, true, "精通伙伴仍可使用夜训的恢复收益");
  const healed = masteredInjured.resolveTarotRest();
  assert.equal(healed.ok, true);
  assert.equal(healed.healed, 6);
  assert.equal(masteredInjured.hp, masteredInjured.maxHp);
  assert.equal(masteredInjured.gold, 20);

  const growingFull = new SemesterGame(517, "gemini");
  growingFull.chooseTarot("strength");
  growingFull.hp = growingFull.maxHp;
  growingFull.pet.bond = 9;
  growingFull.pet.talent = "guardian";
  growingFull.pet.talentLevel = 1;
  const growingPreview = growingFull.tarotRestPreview();
  assert.equal(growingPreview.available, true, "尚未精通时满血也能用夜训推进羁绊");
  assert.equal(growingPreview.before.bond, 9);
  assert.equal(growingPreview.after.bond, 13);
});
...
test("伙伴精通后社团事件不会再出售无效羁绊，精通前仍允许跨过最终里程碑", () => {
  const mastered = new SemesterGame(513, "cancer");
  mastered.gold = 100;
  mastered.pet.bond = 25;
  mastered.pet.talent = "guardian";
  mastered.pet.talentLevel = 3;
  const masteredBefore = mastered.toJSON();

  assert.equal(mastered.activePetIsMastered(), true);
  assert.deepEqual(mastered.eventChoiceStatus("club-pet"), {
    choice: "club-pet",
    available: false,
    reason: "当前伙伴路线已经精通，羁绊不再提供成长",
    actualHeal: null
  });
  assert.deepEqual(mastered.toJSON(), masteredBefore, "读取禁用原因不能扣除校园币或增加羁绊");

  const nearMastery = new SemesterGame(514, "cancer");
  nearMastery.gold = 100;
  nearMastery.pet.bond = 24;
  nearMastery.pet.talent = "guardian";
  nearMastery.pet.talentLevel = 2;
  const preview = nearMastery.eventChoicePreview("club-pet");

  assert.equal(preview.available, true, "尚未精通时仍可购买跨里程碑的最后 2 点羁绊");
  assert.equal(preview.after.gold, 70);
  assert.equal(preview.after.bond, 26);
  nearMastery.pet.bond = preview.after.bond;
  assert.equal(nearMastery.updatePetMilestone(), "master", "跨过 25 羁绊后必须进入精通选择");
});
...
test("期末完成状态可保存恢复，并在进入下一学期后清除", () => {
  ...
  assert.equal(game.completeCurrentSemester(game.semesterUpgradeCandidates()[0].uid), true);
...
test("Boss 胜利后的纪念物与总结升级按阶段保存且不会重复发奖", () => {
  ...
  assert.equal(summaryStage.completeCurrentSemester(summaryStage.semesterUpgradeCandidates()[0].uid), true);
...
test("普通战橡皮擦重抽会原子替换待结算候选并持久化", () => {
  const game = new SemesterGame(3485, "cancer");
  game.chooseTarot("strength");
  const combatStart = game.prepareCombatStart("sleepyBug", "normal");
  game.startCombat(combatStart.enemyId, combatStart.modifiers);
  game.combat.enemy.hp = 0;
  ...
  assert.equal(game.flags.eraserUsed, true);
  assert.equal(rerolled.choices.length, 3);
  assert.equal(rerolled.choices.some((id) => first.includes(id)), false);
  assert.deepEqual(game.pendingCombatReward.choices, rerolled.choices);
  const restored = SemesterGame.fromJSON(game.toJSON());
  assert.deepEqual(restored.pendingCombatReward.choices, rerolled.choices);
  assert.equal(restored.flags.eraserUsed, true);
  ...
  assert.equal(game.rerollPendingCombatReward().ok, false);
  assert.deepEqual(game.pendingCombatReward.choices, spentState.choices);
  assert.equal(game.rng.state, spentState.rng);
  assert.equal(game.rewardIndex, spentState.rewardIndex);
...
test("橡皮擦按候选集合判断变化，单纯换顺序不算新奖励", () => {
  const prepareReward = () => {
    ...
    assert.equal(game.checkCombatEnd(), "won");
    assert.equal(game.completePendingCombatStart(), true);
    const choices = game.prepareNormalCombatReward().choices;
    return { game, choices };
  };

  const probe = prepareReward();
  const reordered = probe.game.rewardCards(3);
  assert.notDeepEqual(reordered, probe.choices);
  assert.deepEqual([...reordered].sort(), [...probe.choices].sort());

  const actual = prepareReward();
  assert.equal(actual.game.addItem("eraser"), true);
  const rerolled = actual.game.rerollPendingCombatReward();
  assert.equal(rerolled.ok, true);
  assert.notDeepEqual([...rerolled.choices].sort(), [...actual.choices].sort());
  assert.equal(rerolled.choices.some((id) => actual.choices.includes(id)), false);
});

test("橡皮擦无法生成完整新集合时回滚随机数、奖励序号与待领取状态", () => {
  const game = new SemesterGame(904, "cancer");
  game.chooseTarot("strength");
  const combatStart = game.prepareCombatStart("sleepyBug", "normal");
  game.startCombat(combatStart.enemyId, combatStart.modifiers);
  game.combat.enemy.hp = 0;
  ...
  const noItemState = { rng: game.rng.state, rewardIndex: game.rewardIndex };
  const noItem = game.rerollPendingCombatReward();
  assert.equal(noItem.ok, false);
  assert.match(noItem.reason, /橡皮擦/);
  assert.deepEqual(game.pendingCombatReward.choices, first);
  assert.equal(game.rng.state, noItemState.rng);
  assert.equal(game.rewardIndex, noItemState.rewardIndex);
  ...
  const before = {
    ...
    rng: game.rng.state,
    rewardIndex: game.rewardIndex,
    eraserUsed: game.flags.eraserUsed
  };
  game.rewardCards = function rewardOnlyReorders(count, rarity, excludedIds) {
    assert.equal(count, 3);
    assert.equal(rarity, null);
    assert.deepEqual(excludedIds, before.choices);
    this.rewardIndex += 1;
    this.rng.next();
    return [...before.choices].reverse();
  };

  const failed = game.rerollPendingCombatReward();
  assert.equal(failed.ok, false);
  assert.match(failed.reason, /未消耗/);
  assert.deepEqual(game.pendingCombatReward.choices, before.choices);
  assert.equal(game.rng.state, before.rng);
  assert.equal(game.rewardIndex, before.rewardIndex);
  assert.equal(game.flags.eraserUsed, before.eraserUsed);
});
...
test("精英战的卡牌、物品与刻印奖励可逐段保存，恢复时不重复发币或重抽", () => {
  ...
  winEliteForRewards(game, usedCardUids);
  const goldBefore = game.gold;
  ...
  assert.equal(restoredAtEnchant.resolvePendingEnchantment(usedCardUids[0])?.status, "enchanted");
...
test("精英物品收齐时只发放一次替代校园币，并直接进入刻印阶段", () => {
  ...
  winEliteForRewards(game, [game.deck[0].uid]);
  const goldBefore = game.gold;
  ...
  assert.equal(restored.loadRepairs.some((note) => /精英刻印候选/.test(note)), false);
...
test("战斗与期末物品奖励共用可取消的整理书包状态", () => {
  ...
  winChallengeForRewards(combat, { affix: Object.keys(CHALLENGE_AFFIX_DEFS)[0] });
  ...
  winFinalExamForRewards(semester);
...
test("战斗开局检查点会重建同一首手牌，并且临时状态只消耗一次", () => {
  ...
  assert.equal(restored.prepareNormalCombatReward(), null, "战斗仍在进行时不能提前生成普通战奖励");
});
