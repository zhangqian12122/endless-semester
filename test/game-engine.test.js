...
test("战斗冲击反馈只展示实际结算差值并区分攻防与敌方行动", () => {
  ...
  assert.equal(skill.motionType, "skill");
...
test("战场意图移除黑色大底，回合规则回到新生教学", () => {
  ...
  assert.match(appSource, /aria-label="敌人意图：\$\{escapeHtml\(detail\)\}"/);
...
test("敌人意图说明按真实字段展开每段攻击、走神和状态牌去向", () => {
  ...
  assert.match(styles, /@media \(max-width: 700px\)[\s\S]*?\.enemy-intent-detail \{[^}]*width: min\(210px, 50vw\);/);
...
test("随机池新增群聊99+与卡纸打印机，且机制按公开意图执行", () => {
  ...
  assert.deepEqual(compositeResult.attack, { perHit: 5, hits: 1, blocked: 0, healthDamage: 5 });
...
test("群聊未读压力按紧张数量即时增加轰炸段数，清理后预告与结算同步降低", () => {
  ...
  assert.deepEqual(resolved.attack, { perHit: relieved.attack, hits: relieved.hits, blocked: 6, healthDamage: 0 });
...
test("动态敌人意图在学期与挑战倍率下仍让预告和真实结算完全一致", () => {
  ...
  assert.equal(printerIntent.attack, 16);
